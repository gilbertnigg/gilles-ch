	...
	<?php foreach ($article->children()->listed()->sortBy('date', 'desc') as $work) : ?>
		...
		<div class="px-1 mb-2 w-1/2 md:w-1/3 md:px-2 md:mb-4 lg:w-1/4">
			<figure class="portfolio__thumb relative"
				...
				data-headline="<?= page($work->type()->value())->title() ?>"
				data-work="<?= $work->uid() ?>">
				...
				<figcaption class="absolute bottom-0 inset-x-0 px-3 py-2 text-left text-xs bg-white sm:text-sm">
					...
					<span class="text-gb"><?= page($work->type()->value())->title()  ?> <?= $work->date()->toDate('Y') ?></span>
				</figcaption>
			</figure>
      	</div>
      <?php endforeach ?>
...
<article>
	...
	<div class="flex mt-4">
		<div class="lg:w-3/4 md:pr-2">
			<ul class="tags">
			<?php $a_kunden = [];
			foreach ($article->children()->filterBy('zeige_kunde', true)->sortBy('kunde', 'asc') as $work) :
